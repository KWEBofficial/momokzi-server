...
latitude = data_from_frontend['latitude'] #위도
longitude = data_from_frontend['longitude'] #경도
user_location = (latitude, longitude)
distanceStd = data_from_frontend['distanceStd'] #500m 기준으로 설정
reviewCountStd = data_from_frontend['reviewCountStd'] #리뷰 필터 0명 기준
opentimeStd = data_from_frontend['opentimeStd'] #현재 운영중, 0이면 조건 설정 없음, 1이면 운영중인지, 2이면 1시간 뒤까지 운영하는지
foodTypeStd = data_from_frontend['foodTypeStd'] #음식종류 제한 없음
starStd = data_from_frontend['starStd'] #현재 별점 제한 없음
locationName = data_from_frontend['locationName'] #이것도 받아오기

# #나중에 받아올 변수들 
# latitude = 37.5846557 #위도
# longitude = 127.0292881 #경도
# user_location = (latitude, longitude)
...
while i < 10:
    ...
    if not df.empty:
        ...
        print("추천 식당: {recommended_restaurant['상호명']}, 행정동명: {recommended_restaurant['행정동명']}")
        #식당 정보 찾아오기
        ...
        # id 가져오기
        print("id:", selected_restaurant["id"])
        # 이름에 해당하는 내용 가져오기
        ...
        reviewNum = 0
        if store_review1:
            store_review1 = str(store_review1.text)
            store_review1 = store_review1.replace(",", "")
            reviewNum += int(store_review1[6:])
        if store_review2:
            store_review2 = str(store_review2.text)
            store_review2 = store_review2.replace(",", "")
            reviewNum += int(store_review2[6:])
        ...
        #json 파일 작성
        json_data = {
            "placeId":selected_restaurant["id"],
